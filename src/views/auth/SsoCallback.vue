<template>
  <div class="sso-callback">
    <div v-if="isLoading" class="loading-container">
      ...
      <p>{{ $t('DE_AU_SSO_PROC', '正在处理认证...') }}</p>
    </div>

    <div v-else-if="error" class="error-container">
      <h2>{{ $t('DE_AU_SSO_FAIL', '认证失败') }}</h2>
      <p>{{ error }}</p>
      <button @click="retry" class="retry-button">{{ $t('G2_BTN_RETRY', '重试') }}</button>
      <button @click="goToLogin" class="login-button">{{ $t('DE_AU_BACK_LOGIN', '返回登录') }}</button>
    </div>

    <div v-else class="success-container">
      <h2>{{ $t('DE_AU_SSO_OK', '认证成功') }}</h2>
      <p>{{ $t('DE_AU_SSO_REDIRECT', '正在跳转到应用...') }}</p>
    </div>
  </div>
</template>
...
const processCallback = async () => {
  try {
    ...
    if (!code || !clientId) {
      throw new Error(t('DE_AU_NO_CODE', '未找到授权码或者客户端ID'));
    }
    ...
    error.value = err instanceof Error ? err.message : t('DE_AU_SSO_ERR', '认证处理失败');
    isLoading.value = false;
  }
};
